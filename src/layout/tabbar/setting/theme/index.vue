<template>
  <el-drawer v-model="config.themeDrawer" :before-close="settingCancel" size="250">
    <template #header>
      <h4>布局设置</h4>
    </template>
    <template #default>
      <el-scrollbar>
        <el-form label-position="left" label-width="150px">
          ...
          <el-form-item label="主题颜色">
            <el-color-picker @change="changeTheme" v-model="config.primary"  show-alpha
                             :predefine="predefineColors"/>
          </el-form-item>
          <el-form-item label="暗黑模式">
            <el-switch @change="changeDark" v-model="config.isDark" inline-prompt
                       active-icon="MoonNight" inactive-icon="Sunny"/>
          </el-form-item>
          <el-form-item label="灰色模式">
            <el-switch @change="changeGrayscale" v-model="config.isGrayscale"
                       inline-prompt
                       active-icon="MoonNight" inactive-icon="Sunny"/>
          </el-form-item>
          <el-form-item label="色弱模式">
            <el-switch @change="changeIsInvert" v-model="config.isInvert" inline-prompt
                       active-icon="MoonNight" inactive-icon="Sunny"/>
          </el-form-item>
          <el-divider content-position="left">菜单设置</el-divider>
          <el-form-item label="菜单背景颜色">
            <el-color-picker @change="changeMenuBackground" v-model="config.menuBackground"  show-alpha
                             :predefine="predefineColors"/>
          </el-form-item>
          <el-form-item label="菜单字体颜色">
            <el-color-picker @change="changeMenuTextColor" v-model="config.menuColor"  show-alpha
                             :predefine="predefineColors"/>
          </el-form-item>
          <el-form-item label="菜单高亮颜色">
            <el-color-picker @change="changeMenuActiveTextColor" v-model="config.menuActiveColor"
                             show-alpha
                             :predefine="predefineColors"/>
          </el-form-item>
          <el-divider content-position="left">顶部设置</el-divider>
          <el-form-item label="顶部背景颜色">
            <el-color-picker @change="changeTopBarBackground" v-model="config.topBarBackground"  show-alpha
                             :predefine="predefineColors"/>
          </el-form-item>
          <el-form-item label="顶部字体颜色">
            <el-color-picker @change="changeTopBarTextColor" v-model="config.topBarColor"  show-alpha
                             :predefine="predefineColors"/>
          </el-form-item>
          <el-form-item label="开启菜单折叠">
            <el-switch @change="SET_THEME_CONFIG(config)" v-model="config.allowMenuFold" inline-prompt
                       active-icon="MoonNight" inactive-icon="Sunny"/>
          </el-form-item>
          <el-form-item label="开启顶部图标">
            <el-switch @change="SET_THEME_CONFIG(config)" v-model="config.topBarIcon" inline-prompt
                       active-icon="MoonNight" inactive-icon="Sunny"/>
          </el-form-item>
          <el-divider content-position="left">页面设置</el-divider>
          <el-form-item label="菜单折叠">
            <el-switch @change="SET_THEME_CONFIG(config)" v-model="config.isCollapse" inline-prompt
                       active-icon="MoonNight" inactive-icon="Sunny"/>
          </el-form-item>
          <el-form-item label="侧边栏Logo">
            <el-switch @change="SET_THEME_CONFIG(config)" v-model="config.asideLogo" inline-prompt
                       active-icon="MoonNight" inactive-icon="Sunny"/>
          </el-form-item>
          <el-form-item label="开启标签页">
            <el-switch @change="SET_THEME_CONFIG(config)" v-model="config.allowTabs" inline-prompt
                       active-icon="MoonNight" inactive-icon="Sunny"/>
          </el-form-item>
          <el-form-item label="开启标签页图标">
            <el-switch @change="SET_THEME_CONFIG(config)" v-model="config.allowTabsIcon" inline-prompt
                       active-icon="MoonNight" inactive-icon="Sunny"/>
          </el-form-item>
          <el-form-item label="开启水印">
            <el-switch @change="changeWartermark" v-model="config.allowWartermark" inline-prompt
                       active-icon="MoonNight" inactive-icon="Sunny"/>
          </el-form-item>
          <el-form-item label="开启页脚">
            <el-switch @change="changeFooter" v-model="config.openFooter" inline-prompt
                       active-icon="MoonNight" inactive-icon="Sunny"/>
          </el-form-item>
          <el-button class="copy-config-btn-reset" type="info" @click="restoreDefaultConfig">
...
import {REMOVE_THEME_CONFIG, SET_THEME_CONFIG} from "@/utils/themeConfigUtil.ts";
