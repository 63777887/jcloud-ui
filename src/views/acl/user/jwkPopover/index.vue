<template>
  <el-popover placement="right" :width="350" trigger="click"  @hide="pswPopoverCancel" :visible="pswPopoverVisible"
              @show="showPswPopoverAction(row)" ref="popoverRef" :virtual-ref="userFormRef">
    <template #reference>
      <div>
        <el-tooltip effect="dark" content="修改密码" placement="top" :enterable="false">
          <el-button v-auth="'sys_user_update'" text type="primary" icon="RefreshLeft" size="small" @click="showPswPopover">密码</el-button>
        </el-tooltip>
      </div>
    </template>
    <el-form :model="userParams" :rules="userRules" ref="userFormRef" label-width="100px"  v-click-outside="onClickOutside">
      <el-form-item label-width="10px" prop="password">
        <el-col :span="16">
          <el-input v-model="userParams.password" show-password clearable placeholder="请输入用户密码"></el-input>
        </el-col>
        <el-col :span="2">
