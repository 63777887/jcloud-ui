<template>
  <el-card>
    ...
    <jwk-upload @close="closeUpload" :show="showUpload" :multiple="false" :size="500" size-type="MB"
                :before-upload="beforeUpload" title="用户导入" :http-request="httpRequest"
                accept="application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"></jwk-upload>
    <jwk-export @confirm="onConfirmHandler" @close="closeExport" :default-export="defaultExportField"
                :data="exportField" title="用户导出" :show="showExport"></jwk-export>
  </el-card>
</template>
...
import {
  reqAddOrUpdateUser,
  reqDelUserBatch, reqExportUser,
  reqGetUserInfo, reqImportUser,
  reqRemoveUser,
  reqUserListByPage,
} from '@/api/acl/user'
import {User, UserReq, UserResponseData} from '@/api/acl/user/type'
import {ElMessage, ElMessageBox, UploadRawFile, UploadRequestOptions} from 'element-plus'
