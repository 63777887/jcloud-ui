  ...
  <el-card>
    <el-form :inline="true" class="searchRole">
      <el-form-item label="角色名">
        <el-input placeholder="请输入角色名" v-model="keyword" @change="getHasRole" clearable></el-input>
      </el-form-item>
      <el-form-item>
        <el-button type="primary" size="default" :disabled="!keyword.length > 0" @click="getHasRole">搜索
        </el-button>
        <el-button size="default" @click="reset">重置</el-button>
      </el-form-item>
    </el-form>
    <div class="searchButton">
      <el-button v-auth="'sys_role_add'" type="primary" size="default" @click="showAddRole">添加角色</el-button>
    </div>
    ...
    <el-table border :data="roleArr">
      ...
      <el-table-column label="角色标识" prop="code" align="center" show-overflow-tooltip></el-table-column>
      ...
      <el-table-column label="操作" width="240px" align="center">
        <template #="{row,$index}">
          <el-tooltip class="item" effect="dark" content="编辑角色" placement="top" :enterable="false">
            <el-button v-auth="'sys_role_update'" text  type="primary" icon="Edit" size="small" @click="showUpdateRole(row)">编辑</el-button>
          </el-tooltip>
          <el-tooltip class="item" effect="dark" content="删除角色" placement="top" :enterable="false">
            <el-button v-auth="'sys_role_delete'" text type="primary" icon="Delete" size="small" @click="delRole(row)">删除</el-button>
          </el-tooltip>
          <el-tooltip class="item" effect="dark" content="分配权限" placement="top" :enterable="false">
            <el-button v-auth="'sys_role_update-menu'" text type="primary" icon="User" size="small"
                       @click="showUpdatePermissions(row)">权限</el-button>
          </el-tooltip>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog
        ...
        @close="roleCancel"
    >
      <el-form :model="roleParams" :rules="roleRules" ref="roleFormRef" label-width="80px">
        <el-form-item label="角色名称" prop="roleName">
          <el-input v-model="roleParams.roleName" placeholder="请输入角色名称"></el-input>
        </el-form-item>
        <el-form-item label="角色标识" prop="code">
          <el-input v-model="roleParams.code" placeholder="请输入角色标识"></el-input>
        </el-form-item>
        <el-form-item label="角色描述" prop="roleDesc">
          <el-input v-model="roleParams.roleDesc" type="textarea" :autosize="{ minRows: 3, maxRows: 6 }"
    ...
    <el-drawer v-model="permissionsDrawer" :before-close="permissionsCancel">
      ...
      <template #default>
        <div class="menu-tree">
          <div>
            <el-button class="change-menu-button" round type="primary" size="default" @click="getMenuTreeByRole(1)">
              菜单权限
            </el-button>
            <el-button class="change-menu-button" round type="primary" size="default" @click="getMenuTreeByRole(2)">
              按钮权限
            </el-button>
          </div>
          <el-input class="filter-menu-input" v-model="filterText" clearable
                    @change="changeTreeInput" @clear="changeTreeInput"
                    :placeholder="selectedMenuType === 1? '请输入菜单名称':'请输入按钮名称'"/>
          <el-tree
...
import {reqAddOrUpdateRole, reqGetRoleInfo, reqRemoveRole, reqRoleListByPage} from "@/api/acl/role";
...
import {reqGetMenuListByRole, reqGetMenuTree, reqSetPermisstion} from "@/api/acl/menu";
...
const delRole = (row: RoleData) => {
  ElMessageBox.confirm(`此操作将永久删除该角色: ${row.roleName}，是否继续`, '提示', {
    ...
    if (result.code == SUCCESS_CODE) {
      ...
      pageNo.value = roleArr.value.length > 1 ? pageNo.value : pageNo.value - 1
      await getHasRole()
    } else {
...
const showUpdatePermissions = async (row: RoleData) => {
  ...
  if (responseData.data) {
    ...
    defaultCheckedKeys.value = filterSelectArr(permissionsData.value, hasMenuIds, [])
  }
...
const getMenuTreeByRole = async (menuType: number) => {
  ...
  const allMenuResponseData = await reqGetMenuTree("", menuType == 2 ? 0 : menuType);
  if (allMenuResponseData.code != SUCCESS_CODE) {
    ElMessage.error(allMenuResponseData.msg)
    return
  }
  ...
  const responseData = await reqGetMenuListByRole(roleParams.id as number, menuType);
  if (responseData.code != SUCCESS_CODE) {
    ElMessage.error(responseData.msg)
    return
  }
...
async function getHasRole() {
  const roleResponseData: RoleResponseData = await reqRoleListByPage(pageNo.value, pageSize.value, keyword.value);
