    ...
    <el-row :gutter="20" class="redis-info">
      ...
      <el-col :span="8">
        <el-card shadow="hover">
          <p>RDB快照的状态</p>
          <h4 class="text-big">
            {{ redisInfo.rdb_last_bgsave_status == 'ok' ? '正常' : '失败' }}
          </h4>
        </el-card>
      </el-col>
      <el-col :span="8">
      ...
      <el-col :span="8">
        <el-card shadow="hover">
          <p>服务器总内存</p>
          <h4 class="text-big">
            {{ formatDataSize(Number(redisInfo.total_system_memory), 'B') }}
          </h4>
        </el-card>
      </el-col>
    </el-row>
...
onMounted(async () => {
  ...
  memoryChartsInit()
  const commandData = Object.entries(redisInfoReq.data.command).map(
  ...
  const keysData = Object.entries(redisInfoReq.data.keys).map(
    ([key, value]) => ({ name: key, value: value }),
  )
