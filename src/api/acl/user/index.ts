...
enum API {
    ...
    // 用户导入
    IMPORT_USER_URL = '/excel/importExcel',
    // 用户导入
    EXPORT_USER_URL = '/excel/exportExcel',
    DOWNLOAD_EXCEL_TEMP_URL = '/excel/downloadExcelTemp',
}
...
//用户导入的接口
export const reqImportUser: ResponseData = (file: UploadRawFile, businessType: String) => {
...
export const reqExportUser: ResponseData = (excelReq: ExportReq) =>
    adminRequest
        .post(API.EXPORT_USER_URL, excelReq, {responseType: 'blob'})
        .then((response) => {
...
//用户导出的接口
export const reqExportTemp: ResponseData = (businessType: String) =>
    adminRequest
        .get(API.DOWNLOAD_EXCEL_TEMP_URL, {
            params: {businessType},
            responseType: 'blob'
        })
        .then((response) => {
