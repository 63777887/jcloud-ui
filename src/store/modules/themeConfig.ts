...
let useThemeConfig = defineStore('themeConfig', {
    ...
    actions: {
        ...
        syncMenuBackground(color?: string) {
            ...
            const lightLevel = 3
            document.documentElement.style.setProperty("--next-bg-menuBackground", color);
            document.documentElement.style.setProperty("--el-menu-hover-bg-color", getLightColor(color, lightLevel));
            document.documentElement.style.setProperty("--next-bg-menuBackground-light-1", getLightColor(color, lightLevel));
        },
        syncDark() {
        ...
        syncMenuTextColor() {
            document.documentElement.style.setProperty("--el-menu-text-color", this.config.menuColor);
        },
        syncMenuActiveTextColor() {
            document.documentElement.style.setProperty("--el-menu-active-color", this.config.menuActiveColor);
        },
        syncTopBarBackground() {
            document.documentElement.style.setProperty("--next-bg-topBarBackground", this.config.topBarBackground);
        },
        syncTopBarTextColor() {
            const lightLevel = 50
            document.documentElement.style.setProperty("--next-bg-topBarColor", this.config.topBarColor);
            document.documentElement.style.setProperty("--next-bg-topBarColor-light-1", getLightColor(this.config.topBarColor, lightLevel));
        },
        syncWartermark(wartermarkText: string) {
            this.config.allowWartermark ? watermark.set(wartermarkText) : watermark.del();
        },
